...
REPO_URL="${REPO_URL:-https://github.com/BoosaIDonng/douyin-sparkflow.git}"
BRANCH="${BRANCH:-main}"
...
*/20 10-17 * * * env SPARKFLOW_TRIGGER_LABEL='scheduled send' bash /app/scripts/run_scheduled_task.sh >> /app/logs/scheduled-task.log 2>&1
0 18 * * * env SPARKFLOW_TRIGGER_LABEL='scheduled send' bash /app/scripts/run_scheduled_task.sh >> /app/logs/scheduled-task.log 2>&1
20 18 * * * env SPARKFLOW_MANUAL_RUN=1 SPARKFLOW_MANUAL_UNSENT_ONLY=1 PYTHONUNBUFFERED=1 SPARKFLOW_TRIGGER_LABEL='unsent fallback' bash /app/scripts/run_scheduled_task.sh >> /app/logs/scheduled-task.log 2>&1
CRON
